...
PARAM$train$undersampling <- 1

# oversampling de los "BAJA+1"  y "BAJA+2"
#  poner 1 si no se quiere oversampling
#  debe ser un numero entero
PARAM$train$oversampling <- 10L

PARAM$train$semilla <- 936737 # cambiar por su propia semilla  !!!
...
# oversampling de los "BAJA+1" y "BAJA+2"
if( PARAM$train$oversampling > 1L )
{
    dtrain_over <- copy( dataset[ foto_mes %in% PARAM$train$training &
          clase_ternaria %in% c("BAJA+1", "BAJA+2") ] )

    dtrain_over[ , fold_validate := 0L ]
    dtrain_over[ , fold_test := 0L ]
    dtrain_over[ , fold_train := 1L ]

    for( i in seq( PARAM$train$oversampling ) )
      dataset  <- rbindlist(list(dataset, dtrain_over))
}
